...
https://github.com/tensorflow/tensorflow/tree/master/tensorflow/examples/image_retraing/retrain.py - 없다
...
* Deep Q-network, 강화학습으로 유면한 Q-러닝을 딥러닝으로 구현했다는 의미
* 강화학습 - 어떤 환경에서 인공지능 에이전트가 현재 상태를 판단하여 가장 이로운 행동을 하게 만드는 학습방법입니다. 이로운 행동하면 보상, 해로운 행동하면 페널티
* Q-러닝 - 어떤 상태에서 특정 행동을 했을 때의 가치를 나타내는 함수인 Q함수를 학습하는 알고리즘, 특정 상태에서 이 함수의 값이 최대가 되게 학습하며 어떤 행동 취할지 알수 있게 한다. 딥러닝으로 구현시 불안정, 두 방법으로 해결
* 과거의 상태를 기억한 뒤 그 중 임의의 상태를 뽑아 학습시키는 방법 사용, 특수 상황에 치우치지 않도록 학습 가능
* 손실값 계산을 위해 학습을 진행하면서 최적의 행동을 얻는 기본 신경망과 진짜 좋은지 비교하는 목표 신경망 분리
* DQN은 영상 상태만 인식하므로 CNN으로 구성

### 게임소개

* OpenAI - 인공지능 연구하는 비영리 회사, Gym(https://gym.openai.com/)제공중, 다양한 실험 가능, 아타리 게임들을 쉽게 구동할 수 있다. 다만 학습에는 고성능 PC와 긴 시간 필요
